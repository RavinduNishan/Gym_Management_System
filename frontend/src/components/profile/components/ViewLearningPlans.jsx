...
const ViewUserLearningPlans = () => {
  ...
  return (
    <div className="min-h-screen bg-PrimaryColor font-sans">
      <Navbar user={currentUser} />
      <div className="max-w-5xl mx-auto mt-6 px-4 sm:px-6 lg:px-8 animate-fadeIn">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-semibold text-ExtraDarkColor">My Learning Plans</h1>
          {currentUser && (
           <button
           onClick={() => navigate('/learning-plans/create')}
           className="px-4 py-2 bg-DarkColor text-PrimaryColor rounded-md hover:bg-ExtraDarkColor transition-colors duration-1500 delay-300 font-sans font-medium animate-fadeIn"
         >
           Create New Plan
         </button>
        ...
        {isLoadingPlans ? (
          <div className="flex justify-center items-center py-6">
            <div className="animate-spin rounded-full h-8 w-8 border-t-4 border-b-4 border-DarkColor"></div>
          </div>
        ) : learningPlans.length === 0 ? (
          <div className="text-center py-6">
            <p className="text-SecondaryColor">No learning plans found. Create one to get started!</p>
          </div>
        ) : (
          <div className="space-y-6">
            {learningPlans.map((plan) => (
              <div key={plan.id} className="bg-white shadow-md rounded-lg p-6 border border-SecondaryColor/20">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold text-ExtraDarkColor">{plan.title || 'Untitled'}</h2>
                  <div className="space-x-2">
                    <button
                      onClick={() => handleEdit(plan.id)}
                      className="text-DarkColor hover:text-ExtraDarkColor transition-colors duration-300"
                    >
                      Edit
                    </button>
                    <button
                ...
                <p className="text-SecondaryColor mb-4">{plan.description || 'No description'}</p>
                <div className="text-SecondaryColor mb-4">
                  <span>
                    Duration:{' '}
                ...
                {plan.resources && Array.isArray(plan.resources) && plan.resources.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-lg font-medium text-ExtraDarkColor mb-2">Resources</h3>
                    <ul className="list-disc pl-5 space-y-1">
                      {plan.resources.map((resource, index) => (
                        <li key={index} className="text-SecondaryColor">
                          <a
                            href={resource.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-DarkColor hover:underline transition-colors duration-300"
                          >
                ...
                {plan.weeks && Array.isArray(plan.weeks) && plan.weeks.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-lg font-medium text-ExtraDarkColor mb-2">Timeline</h3>
                    <div className="space-y-4">
                      {plan.weeks.map((week, index) => (
                        <div key={index} className="border-l-2 border-DarkColor pl-4">
                          <h4 className="text-sm font-medium text-DarkColor">
                            {week.title || `Week ${index + 1}`}
                          </h4>
                          <p className="text-SecondaryColor">{week.description || 'No description'}</p>
